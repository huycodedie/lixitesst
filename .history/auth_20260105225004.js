...
function updateAuthUI() {
  ...
  if (userMenu) {
    ...
    if (user) {
      displayName = user.name || user.fullname || user.full_name || user.email || null;
    ...
    if (!displayName && tokenPresent) {
      try {
        ...
        if (payload) {
          displayName = payload.name || payload.fullname || payload.full_name || payload.username || payload.email || null;
