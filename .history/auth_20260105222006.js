...
const AuthManager = {
  ...
  async login(email, password) {
    ...
    if (!response.ok || data.success === false || data.error || (data.message && data.message.includes("không tìm thấy"))) {
    ...
    if (data.access_token) {
      ...
      if (userId) {
        try {
          const userRes = await fetch(`${this.API_URL}/user/get-details-user/${userId}`, {
...
function updateAuthUI() {
  ...
  if (token || user) {
    ...
    if (userMenu) {
      ...
      if (userName && user) userName.textContent = user.name || user.email || 'User';
      if (userAvatar && user) {
        userAvatar.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(user.name || user.email)}&background=EF4444&color=fff`;
      }
    }
  } else {
