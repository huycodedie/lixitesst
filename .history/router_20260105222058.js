...
const Router = {
  ...
  async loadHomeEnvelopes() {
    ...
    try {
      ...
      const res = await fetch(`${AuthManager.API_URL}/envelope/list`, {
      ...
      if (envelopes.length === 0) {
        status.textContent = '📭 Chưa có phong bì nào';
        tbody.innerHTML = '';
        table.style.display = 'none';
      } else {
        status.textContent = '';
        ...
        tbody.innerHTML = envelopes.map((env, i) => `
          <tr class="text-center border-t">
            ...
            <td class="px-4 py-2 border">${env._id || env.id || ''}</td>
            <td class="px-4 py-2 border">${env.recipientName || env.name || ''}</td>
            <td class="px-4 py-2 border">${(env.amount || 0).toLocaleString()} VNĐ</td>
            <td class="px-4 py-2 border">${new Date(env.createdAt || env.created || Date.now()).toLocaleDateString('vi-VN')}</td>
          </tr>
        `).join('');
      }
    } catch (err) {
      status.textContent = '❌ ' + (err.message || 'Lỗi');
      table.style.display = 'none';
      tbody.innerHTML = '';
    }
  },
  ...
  async loadEnvelopes() {
    ...
    if (!listContainer || !status) return;
          ...
          <div class="bg-yellow-100 rounded-lg p-4 text-gray-800">
            <div class="flex justify-between items-center">
              <div>
                ...
                <p class="text-xs text-gray-600">📅 ${new Date(env.createdAt).toLocaleDateString('vi-VN')}</p>
              </div>
            </div>
          </div>
        `).join('');
        ...
        tbody.innerHTML = envelopes
          ...
          <tr class="text-center border-t">
            ...
            <td class="px-4 py-2 border">${
              env.recipientName || env.name || ""
            }</td>
            <td class="px-4 py-2 border">${(
              env.amount || 0
            ).toLocaleString()} VNĐ</td>
            <td class="px-4 py-2 border">${new Date(
              env.createdAt || env.created || Date.now()
